...
const SimulationTheater: React.FC<TheaterProps> = ({ 
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col gap-4">
      ...
      <div 
        ref={scrollRef}
        className="flex-1 min-h-0 bg-[rgba(10,16,30,0.7)] border border-white/10 rounded-2xl overflow-y-auto p-6 flex flex-col gap-6 custom-scrollbar scroll-region shadow-[0_24px_50px_rgba(6,10,20,0.35)]"
      >
        {messages.length === 0 && !isProcessing && (
          <div className="h-full flex flex-col items-center justify-center text-center opacity-40">
            <div className="w-24 h-24 border border-cyan-400/30 rounded-full flex items-center justify-center mb-6 shadow-[0_12px_24px_rgba(93,212,255,0.2)]">
          ...
          return (
            <div key={msg.id} className={`flex flex-col gap-2 max-w-[85%] ${persona?.color === '#ef4444' ? 'self-end items-end' : 'self-start items-start'}`}>
              ...
              <div className="group relative">
                <div 
                  className="px-5 py-4 rounded-2xl bg-white/5 border border-white/10 text-white/90 text-sm leading-relaxed shadow-[0_14px_30px_rgba(7,12,22,0.45)]"
                ...
                {/* Meta Overlay */}
                <div className="absolute -bottom-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity bg-[rgba(8,12,24,0.95)] border border-white/10 rounded-xl px-2 py-0.5 flex gap-3 text-[9px] mono text-[var(--text-subtle)]">
        ...
        {isProcessing && streamingText && (
          <div className="flex flex-col gap-2 max-w-[85%] self-start items-start fade-slide-in">
            ...
            <div className="px-5 py-4 rounded-2xl bg-cyan-500/10 border border-cyan-400/30 text-white/80 text-sm italic leading-relaxed shadow-[0_12px_24px_rgba(93,212,255,0.15)]">
              {streamingText}
...
const StatCard = ({ label, value, color }: { label: string, value: string, color: string }) => (
  <div className="bg-[rgba(11,19,36,0.85)] border border-white/10 rounded-2xl p-4 shadow-[0_16px_30px_rgba(6,10,20,0.3)]">
    <div className="text-[10px] text-[var(--text-subtle)] font-bold uppercase tracking-widest mb-1">{label}</div>
    <div className="text-xl font-bold mono" style={{ color: STAT_COLORS[color] || STAT_COLORS.white }}>
      {value}
    </div>
  </div>
);
